...
export var settingsKeys = {
  all: ['settings'],
  salon: function(salonId) { return ['settings', 'salon', salonId]; },
  widget: function(salonId) { return ['settings', 'widget', salonId]; },
  notifications: function(salonId) { return ['settings', 'notifications', salonId]; },
  policies: function(salonId) { return ['settings', 'policies', salonId]; },
};
...
export var CANCELLATION_POLICIES = [
  { value: 'flexible', label: 'Flexible', description: 'Free cancellation up to 1 hour before' },
  { value: 'moderate', label: 'Moderate', description: 'Free cancellation up to 24 hours before' },
  { value: 'strict', label: 'Strict', description: 'Free cancellation up to 48 hours before' },
  { value: 'custom', label: 'Custom', description: 'Define your own cancellation window' },
];
...
export function useUpdateSalonSettings() {
  ...
  return useMutation({
    ...
    onSuccess: function(data, variables) {
      queryClient.invalidateQueries({ queryKey: settingsKeys.salon(variables.salonId) });
    },
  });
}
...
export function useUpdateSalonPolicies() {
  ...
  return useMutation({
    mutationFn: async function(params) {
      var response = await fetch('/api/salons/' + params.salonId + '/settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(params.data),
    ...
    onSuccess: function(data, variables) {
      queryClient.invalidateQueries({ queryKey: settingsKeys.salon(variables.salonId) });
      queryClient.invalidateQueries({ queryKey: settingsKeys.policies(variables.salonId) });
    },
  });
}
...
export function useUpdateWidgetSettings() {
  ...
  return useMutation({
    ...
    onSuccess: function(data, variables) {
      queryClient.invalidateQueries({ queryKey: settingsKeys.widget(variables.salonId) });
    },
  });
}
...
export function useDeleteSalonPhoto() {
  ...
  return useMutation({
    mutationFn: async function(params) {
      var response = await fetch('/api/salons/' + params.salonId + '/photos/' + params.photoId, {
        method: 'DELETE',
        credentials: 'include',
      });
...
export function useToggleMarketplace() {
  ...
  return useMutation({
    mutationFn: async function(params) {
      ...
      var response = await fetch('/api/salons/' + params.salonId + '/marketplace/' + action, {
        method: 'POST',
        credentials: 'include',
      });
...
export function generateEmbedCode(salonId, options) {
  ...
  if (options.showServices !== undefined) params.set('services', options.showServices);
  
  var queryString = params.toString();
  var widgetUrl = baseUrl + '/widget/' + salonId + (queryString ? '?' + queryString : '');
  
  return {
    iframe: '<iframe src="' + widgetUrl + '" width="100%" height="600" frameborder="0"></iframe>',
    script: '<script src="' + baseUrl + '/widget.js" data-salon-id="' + salonId + '"></script>',
    link: widgetUrl,
  };
}
