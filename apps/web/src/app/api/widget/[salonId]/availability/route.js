...
export async function GET(request, { params }) {
  try {
    ...
    const serviceStaffPairs = servicesParam.split(',').map(pair => {
      ...
      return { serviceId: parseInt(serviceId), staffId: parseInt(staffId) || null };
    });
    ...
    // Get the earliest start time and latest end time across all staff
    ...
    for (const staffId of staffIds) {
      ...
      if (timeOff) {
        // If any staff has time off, no slots available
        return success({ slots: [] });
      }
    ...
    // Get existing bookings for all staff on this date
    const staffBookings = {};
    for (const staffId of staffIds) {
      const bookings = await query(
        `SELECT b.start_datetime, b.end_datetime 
         ...
         WHERE bs.staff_id = ? AND DATE(b.start_datetime) = ? 
         AND b.status NOT IN ('cancelled', 'no_show')
         ORDER BY b.start_datetime`,
        [staffId, date]
      );
      staffBookings[staffId] = bookings;
    }
