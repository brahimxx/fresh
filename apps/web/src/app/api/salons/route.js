...
export async function GET(request) {
  try {
    ...
    let sql = `
      ...
      WHERE s.is_marketplace_enabled = 1
    `;
    const params = [];
    ...
    sql += ' GROUP BY s.id, s.name, s.description, s.phone, s.email, s.address, s.city, s.country, s.latitude, s.longitude, s.timezone, s.currency, s.is_marketplace_enabled, s.created_at';
    ...
    // Get total count
    let countSql = 'SELECT COUNT(DISTINCT s.id) as total FROM salons s WHERE s.is_marketplace_enabled = 1';
    const countParams = [];
    if (city) {
      countSql += ' AND s.city = ?';
