...
export async function POST(request, { params }) {
  try {
    ...
    const user = await getOne('SELECT id, first_name, last_name FROM users WHERE id = ?', [userId]);
    if (!user) {
      return notFound('User not found');
    }

    // Check if already staff at this salon
    const existingStaff = await getOne('SELECT id FROM staff WHERE salon_id = ? AND user_id = ?', [id, userId]);
    if (existingStaff) {
