  ...
  const handleSelect = async (loc) => {
    ...
    if (loc.place_id) {
      if (geocodeCache.has(loc.place_id)) {
        const cacheData = geocodeCache.get(loc.place_id);
        lat = cacheData.lat;
        lng = cacheData.lng;
        address_components = cacheData.address_components;
        ...
        try {
          ...
          if (response.results && response.results[0]) {
            ...
            geocodeCache.set(loc.place_id, { lat, lng, address_components });
          }
        } catch (e) {
        ...
        >
          {isSearching ? (
            ...
            <div className="flex flex-col w-full py-2 px-2">
              <h4 className="text-[13px] font-bold text-popover-foreground px-3 pt-2 pb-2">
                Addresses
              </h4>
              {suggestions.map((loc, i) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2.5 rounded-lg transition-colors flex items-center gap-3 hover:bg-muted mb-1 last:mb-0"
                >
