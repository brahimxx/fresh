...
import { Calendar, Clock, Scissors, Check, XCircle, AlertCircle } from 'lucide-react';
...
var STATUS_CONFIG = {
  ...
  confirmed: { label: 'Confirmed', className: 'bg-green-100 text-green-800', icon: Check },
  completed: { label: 'Completed', className: 'bg-blue-100 text-blue-800', icon: Check },
  cancelled: { label: 'Cancelled', className: 'bg-gray-100 text-gray-800', icon: XCircle },
  no_show: { label: 'No Show', className: 'bg-red-100 text-red-800', icon: AlertCircle },
};

export function ClientBookingHistory({ clientId, salonId }) {
  var router = useRouter();
  var { data: bookings, isLoading } = useClientBookings(clientId);
  
  function handleViewBooking(bookingId) {
    router.push('/dashboard/salon/' + salonId + '/bookings?selected=' + bookingId);
  }
  ...
  if (bookings) {
    bookings.forEach(function(booking) {
      var bookingDate = new Date(booking.start_datetime || booking.startDateTime);
        ...
        {isLoading ? (
                  ...
                  {upcomingBookings.map(function(booking) {
                    ...
                    return (
                      <div 
                        ...
                        onClick={function() { handleViewBooking(booking.id); }}
                      >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              ...
                              {(booking.total_price || booking.totalPrice) && (
                                <>
                                  <span>•</span>
                                  <span>EUR {Number(booking.total_price || booking.totalPrice).toFixed(2)}</span>
                                </>
                              )}
                            </div>
                          </div>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <Button 
              variant="link" 
              className="mt-2"
              onClick={function() { router.push('/dashboard/salon/' + salonId + '/calendar'); }}
            >
              Create a booking
            </Button>
          </div>
