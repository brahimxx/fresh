...
function SearchBarContent({
  ...
  const defaultLocation =
    initialLocationQuery !== undefined
      ? initialLocationQuery
  ...
  const [locationDropdownMaxHeight, setLocationDropdownMaxHeight] =
    useState("60vh");

  // Debounce both inputs
  const debouncedSearchQuery = useDebounce(searchQuery, 300);
  const debouncedLocationQuery = useDebounce(
    isPresetLocationLabel ? "" : locationQuery,
    300,
  );
  ...
  const handleGetLocation = ({
    fallbackToIpOnFailure = true,
    showCurrentLocationWhileResolving = false,
    onFailure = null,
  } = {}) => {
    setIsGettingLocation(true);

    if (showCurrentLocationWhileResolving) {
  ...
  // Auto-select current position by default on the salons listing page.
  // If permission is denied/unavailable, keep the existing default behavior.
  useEffect(() => {
    if (autoLocationAttempted) return;
    if (pathname !== "/salons") return;

    const hasLocationInUrl = !!searchParams.get("location");
    const hasCoordsInUrl = !!(
  ...
  return (
    ...
    >
      ...
      >
        {/* Service Input */}
        <div
          className={`relative flex-1 bg-transparent flex items-center ${isCompact ? "px-3 w-auto" : "px-4 w-full"}`}
        >
          <Search
            className={`text-muted-foreground mr-2 shrink-0 ${isCompact ? "h-3.5 w-3.5" : isLg ? "h-5 w-5" : "h-4 w-4"}`}
          />
          <input
            placeholder={
              isCompact ? "Services or salons..." : "All treatments and venues"
            }
        ...
        {/* Divider */}
        <div
          className={`${isCompact ? "block" : "hidden md:block"} w-px ${isCompact ? "h-5" : "h-8"} bg-border/50 mx-2 shrink-0`}
        />

        {/* Location Input */}
        <div
          className={`relative flex-1 flex items-center ${isCompact ? "px-3 w-auto" : "px-4 w-full border-t border-border/50 md:border-none mt-2 md:mt-0 pt-2 md:pt-0"}`}
        >
          <MapPin
            className={`text-muted-foreground mr-2 shrink-0 ${isCompact ? "h-3.5 w-3.5" : isLg ? "h-5 w-5" : "h-4 w-4"}`}
          />
          <input
            ...
            className={`w-full bg-transparent outline-none border-none focus:ring-0 ${isPresetLocationLabel ? "text-foreground font-medium" : "placeholder:text-muted-foreground"} ${isCompact ? "h-8 text-xs" : isLg ? "h-14 text-base md:text-lg font-medium" : "h-10 text-sm"}`}
            value={locationQuery}
            onChange={(e) => {
              ...
              {!locationQuery.trim() ||
                ...
                <div className="flex flex-col w-full py-4 space-y-4">
                  ...
                  <div className="px-6">
                    ...
                    >
                      <Locate
                        className={`h-5 w-5 text-primary shrink-0 ${isGettingLocation ? "animate-pulse" : ""}`}
                      />
                      ...
                      <div className="space-y-1">
                        {userAddresses.length > 0 ? (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    <span className="text-[14px] font-medium text-popover-foreground block">
                                      {addr.label}
                                    </span>
                                    {addr.isDefault && (
                                      <Badge
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="text-[14px] font-medium text-primary block hover:underline">
                                Add New Address
                              </span>
                            </div>
                          </button>
                      ...
                      <div className="space-y-1">
                        {recentSearches.map((loc, i) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="text-[14px] font-medium text-popover-foreground truncate block">
                                {loc}
                              </span>
                            </div>
                          </button>
            ...
            <div className="flex flex-col w-full pb-4">
              ...
              {matchedServices.length > 0 && (
                <div className="pt-4 px-2">
                  <h4 className="text-[13px] font-bold text-popover-foreground px-4 mb-2">
                    Treatments
                  </h4>
                ...
                <div className="flex items-center">
                  {isManagingAddresses && (
                    <button
                      ...
                      className="text-muted-foreground hover:text-foreground flex items-center gap-1 text-sm font-medium mr-2"
                    >
                  ...
                  <AddressAutocomplete
                    ...
                    onChange={(loc) =>
                      setNewAddressForm({
                        ...
                        lat: loc.lat || newAddressForm.lat,
                        lng: loc.lng || newAddressForm.lng,
                      })
                    }
                ...
                {userAddresses.length > 0 ? (
                  userAddresses.map((addr) => {
                    ...
                    return (
                      <div
                        key={addr.id}
                        className="w-full flex items-center justify-between py-3 border-b border-border/10 last:border-0 group"
                      >
                        <div className="flex items-center gap-4 min-w-0 pr-4">
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-xs text-muted-foreground truncate block pt-0.5">
                              {addr.fullAddress}
                            </span>
                          </div>
                        </div>
