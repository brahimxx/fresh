...
export function SalonSearchContent({ initialCategory, initialCity }) {
  ...
  var [location, setLocation] = useState(searchParams.get('location') || initialCity || '');
            ...
            <div className="sticky top-16 z-30 bg-background -mt-6 pt-6 pb-4 mb-2">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Sheet>
                  ...
                  <SheetContent side="left" className="w-[340px] p-0 flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                      <div>
                        <h4 className="text-sm font-semibold mb-4 text-foreground/80 uppercase tracking-wide">Categories</h4>
                            ...
                            return (
                              <div key={cat.id} className="flex items-center space-x-3 group">
                                <Checkbox
                                  ...
                                  onCheckedChange={function () { toggleCategory(cat.label); }}
                                  className="h-5 w-5 rounded-md border-border text-primary data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                                />
                                <Label htmlFor={'mob-cat-' + cat.slug} className="cursor-pointer text-base font-medium group-hover:text-primary transition-colors flex-1">{cat.label}</Label>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div className="border-t border-border/50 pt-8">
                        <h4 className="text-sm font-semibold mb-4 text-foreground/80 uppercase tracking-wide">Price Range</h4>
                      ...
                      <div className="border-t border-border/50 pt-8">
                        <h4 className="text-sm font-semibold mb-4 text-foreground/80 uppercase tracking-wide">Minimum Rating</h4>
                        <div className="flex flex-wrap gap-2">
                          {[4.0, 4.5, 4.8].map(function (rating) {
                            return (
                              ...
                              >
                                <Star className={`h-3.5 w-3.5 ${minRating === rating ? 'fill-primary-foreground' : 'fill-yellow-400 text-yellow-400'}`} />
                                {rating}+
                              </Button>
                            );
                          })}
                      ...
                      <div className="border-t border-border/50 pt-8 pb-4">
                         <h4 className="text-sm font-semibold mb-4 text-foreground/80 uppercase tracking-wide">Availability</h4>
                        <div className="flex items-center space-x-3 group bg-muted/30 p-3 rounded-xl border border-border/50">
                          ...
                          />
                          <Label htmlFor="mob-open-now" className="cursor-pointer text-base font-medium flex items-center gap-2">
                    ...
                    <div className="p-6 border-t border-border/50 bg-background/80 backdrop-blur-sm sticky bottom-0">
                      <Button variant="outline" onClick={clearFilters} className="w-full h-12 rounded-xl text-base font-medium shadow-sm hover:bg-destructive/5 hover:text-destructive hover:border-destructive/30 transition-all">
                        Clear All Filters
                      </Button>
                    </div>
                  </SheetContent>
              ...
              {activeFilterCount > 0 && (
                <div className="flex items-center gap-2 mt-4 flex-wrap">
                  ...
                  {selectedCategories.map(function (catLabel) {
                    var cat = BUSINESS_CATEGORIES.find(function (c) { return c.label === catLabel; });
                    return (
                      <Badge key={catLabel} variant="secondary" className="gap-1">
                        {cat?.label || catLabel}
                        <X className="h-3 w-3 cursor-pointer" onClick={function () { toggleCategory(catLabel); }} />
                      </Badge>
                    );
                  })}
                  {openNow && (
